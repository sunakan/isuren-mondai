...
# repository-wide mise layout as the other targets instead of a recipe-only
# /opt path. Go 1.26.6 is the only runtime this AMI needs (frontend/initial
# data are already built by scripts/build-frontend-release.sh in CI).
install_mise() {
  if [ -x "${MISE_BIN}" ] &&
