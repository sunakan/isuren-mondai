...
create_user() {
  if ! id -u "${ISUREN_USER}" >/dev/null 2>&1; then
    useradd -u "${ISUREN_UID}" -g "${ISUREN_USER}" -d "/home/${ISUREN_USER}" -m -s /bin/bash "${ISUREN_USER}"
...
# kakomon9-qualify独自: these directories are created before runtime/source
# steps consume the target's application and durable provenance paths.
