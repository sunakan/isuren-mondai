...
fail-fast status propagation. `05-artifacts.sh` validates the complete prepared
dist tree before any file is consumed. `50-source.sh` deploys the maintained Go
source and exact official data assets as real files. `80-frontend.sh` only
places the already prepared official public tree; it never invokes Node.js,
npm, a package manager, minification, or domain replacement.
