...
official project rootはsourceだけでなくApplication、benchmark binary、initial-data、static tree等のruntime contractを束ねることがある。compile-only packageを削除できることからproject root全体を不要と推測せず、runtime参照を残したままofficialの相対pathを保つ。他editionのhome-level binary名やdirectory構成をpractice-image標準として横展開しない。

benchmarkもsource packages、module entrypoint、final binary、任意のlocal helper/wrapper、runtime assetを分ける。helper/wrapperはofficial artifactではなくrecipe独自物として扱い、採用済みconsumer/result契約が必要とする場合だけAMIへ含める。不要なら作らず、direct command、cwd、official引数、最終JSONとexit/failure semanticsをREADME/provenanceへ記録する。sourceを削除してもinitial-data、static tree、fixture、cwd等のruntime参照は残す。

payment/shipment等のmockがbenchmark process内で起動するeditionでは、別のpersistent serviceを作らない。benchmark開始前に対象portが空いていることをfail-fastで確認し、direct binaryまたは明示採用したwrapperがofficial引数を保持する。公式benchmarkが`pass=false`でもexit 0を返す場合、最終JSONを正本とする責任をwrapper、Portal/runner、または人間の実行手順のどこが持つか明記し、未採用のhelperで暗黙に上書きしない。

final pathを変更・本家準拠へ戻す場合は、新pathを作るだけで完了にしない。step再実行時に旧binary/helper/cache pathを明示的に除去し、Gossとstatic validationで旧path不在を観測する。fresh imageだけの検証では既存EC2/Orbへの再provisionで残るstale artifactを検出できない。
