...
- 編集しない画像・静的asset、`sql/`、初期データは原則managed sourceへcommitせず、worktree-local mirrorからfrontend artifact buildまたはbundle準備前に`rsync`する。このimportでは`.git/`、`node_modules/`、`dist/`等を除外し、取得元LICENSEを含め、managed sourceと実ファイルで統合する。
