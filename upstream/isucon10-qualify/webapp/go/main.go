...
// echo v5 dropped the gommon Logger interface (Errorf/Infof) in favor of log/slog,
// which has no printf-style helpers. These wrappers keep call sites unchanged.
func logErrorf(logger *slog.Logger, format string, args ...interface{}) {
	logger.Error(fmt.Sprintf(format, args...))
}

func logInfof(logger *slog.Logger, format string, args ...interface{}) {
...
func main() {
	// Echo instance
	e := echo.New()
	e.Logger = slog.New(slog.NewTextHandler(os.Stderr, &slog.HandlerOptions{Level: slog.LevelDebug}))
...
func initialize(c *echo.Context) error {
	...
	for _, p := range paths {
		...
		if err := exec.Command("bash", "-c", cmdStr).Run(); err != nil {
			logErrorf(c.Logger(), "Initialize script error : %v", err)
...
func getChairDetail(c *echo.Context) error {
	...
	if err != nil {
		logErrorf(c.Echo().Logger, "Request parameter \"id\" parse error : %v", err)
	...
	if err != nil {
		if err == sql.ErrNoRows {
			logInfof(c.Echo().Logger, "requested id's chair not found : %v", id)
			return c.NoContent(http.StatusNotFound)
		}
		logErrorf(c.Echo().Logger, "Failed to get the chair from id : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	} else if chair.Stock <= 0 {
		logInfof(c.Echo().Logger, "requested id's chair is sold out : %v", id)
		return c.NoContent(http.StatusNotFound)
	}
...
func postChair(c *echo.Context) error {
	...
	if err != nil {
		logErrorf(c.Logger(), "failed to get form file: %v", err)
		return c.NoContent(http.StatusBadRequest)
	}
	f, err := header.Open()
	if err != nil {
		logErrorf(c.Logger(), "failed to open form file: %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
	defer f.Close()
	records, err := csv.NewReader(f).ReadAll()
	if err != nil {
		logErrorf(c.Logger(), "failed to read csv: %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}

	tx, err := db.Begin()
	if err != nil {
		logErrorf(c.Logger(), "failed to begin tx: %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
	defer tx.Rollback()
	for _, row := range records {
		...
		if err := rm.Err(); err != nil {
			logErrorf(c.Logger(), "failed to read record: %v", err)
			return c.NoContent(http.StatusBadRequest)
		}
		...
		if err != nil {
			logErrorf(c.Logger(), "failed to insert chair: %v", err)
			return c.NoContent(http.StatusInternalServerError)
		}
	}
	if err := tx.Commit(); err != nil {
		logErrorf(c.Logger(), "failed to commit tx: %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func searchChairs(c *echo.Context) error {
	...
	if c.QueryParam("priceRangeId") != "" {
		...
		if err != nil {
			logInfof(c.Echo().Logger, "priceRangeID invalid, %v : %v", c.QueryParam("priceRangeId"), err)
			return c.NoContent(http.StatusBadRequest)
		}
	...
	if c.QueryParam("heightRangeId") != "" {
		...
		if err != nil {
			logInfof(c.Echo().Logger, "heightRangeIf invalid, %v : %v", c.QueryParam("heightRangeId"), err)
			return c.NoContent(http.StatusBadRequest)
		}
	...
	if c.QueryParam("widthRangeId") != "" {
		...
		if err != nil {
			logInfof(c.Echo().Logger, "widthRangeID invalid, %v : %v", c.QueryParam("widthRangeId"), err)
			return c.NoContent(http.StatusBadRequest)
		}
	...
	if c.QueryParam("depthRangeId") != "" {
		...
		if err != nil {
			logInfof(c.Echo().Logger, "depthRangeId invalid, %v : %v", c.QueryParam("depthRangeId"), err)
			return c.NoContent(http.StatusBadRequest)
		}
	...
	if len(conditions) == 0 {
		logInfof(c.Echo().Logger, "Search condition not found")
		return c.NoContent(http.StatusBadRequest)
	}
	...
	if err != nil {
		logInfof(c.Logger(), "Invalid format page parameter : %v", err)
		return c.NoContent(http.StatusBadRequest)
	}

	perPage, err := strconv.Atoi(c.QueryParam("perPage"))
	if err != nil {
		logInfof(c.Logger(), "Invalid format perPage parameter : %v", err)
		return c.NoContent(http.StatusBadRequest)
	}
	...
	if err != nil {
		logErrorf(c.Logger(), "searchChairs DB execution error : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func buyChair(c *echo.Context) error {
	...
	if err := c.Bind(&m); err != nil {
		logInfof(c.Echo().Logger, "post buy chair failed : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
	...
	tx, err := db.Beginx()
	if err != nil {
		logErrorf(c.Echo().Logger, "failed to create transaction : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
	defer tx.Rollback()
	...
	if err != nil {
		if err == sql.ErrNoRows {
			logInfof(c.Echo().Logger, "buyChair chair id \"%v\" not found", id)
			return c.NoContent(http.StatusNotFound)
		}
		logErrorf(c.Echo().Logger, "DB Execution Error: on getting a chair by id : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}

	_, err = tx.Exec("UPDATE chair SET stock = stock - 1 WHERE id = ?", id)
	if err != nil {
		logErrorf(c.Echo().Logger, "chair stock update failed : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}

	err = tx.Commit()
	if err != nil {
		logErrorf(c.Echo().Logger, "transaction commit error : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func getLowPricedChair(c *echo.Context) error {
	...
	if err != nil {
		...
		logErrorf(c.Logger(), "getLowPricedChair DB execution error : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func getEstateDetail(c *echo.Context) error {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		logInfof(c.Echo().Logger, "Request parameter \"id\" parse error : %v", err)
		return c.NoContent(http.StatusBadRequest)
	}
	...
	if err != nil {
		if err == sql.ErrNoRows {
			logInfof(c.Echo().Logger, "getEstateDetail estate id %v not found", id)
			return c.NoContent(http.StatusNotFound)
		}
		logErrorf(c.Echo().Logger, "Database Execution error : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func postEstate(c *echo.Context) error {
	...
	for _, row := range records {
		...
		if err != nil {
			logErrorf(c.Logger(), "failed to insert estate: %v", err)
			return c.NoContent(http.StatusInternalServerError)
		}
	}
	if err := tx.Commit(); err != nil {
...
func searchEstates(c *echo.Context) error {
	...
	if c.QueryParam("doorHeightRangeId") != "" {
		...
		if err != nil {
			logInfof(c.Echo().Logger, "doorHeightRangeID invalid, %v : %v", c.QueryParam("doorHeightRangeId"), err)
			return c.NoContent(http.StatusBadRequest)
		}
	...
	if c.QueryParam("doorWidthRangeId") != "" {
		...
		if err != nil {
			logInfof(c.Echo().Logger, "doorWidthRangeID invalid, %v : %v", c.QueryParam("doorWidthRangeId"), err)
			return c.NoContent(http.StatusBadRequest)
		}
	...
	if c.QueryParam("rentRangeId") != "" {
		...
		if err != nil {
			logInfof(c.Echo().Logger, "rentRangeID invalid, %v : %v", c.QueryParam("rentRangeId"), err)
			return c.NoContent(http.StatusBadRequest)
		}
	...
	if len(conditions) == 0 {
		logInfof(c.Echo().Logger, "searchEstates search condition not found")
		return c.NoContent(http.StatusBadRequest)
	}
	...
	err = db.Get(&res.Count, countQuery+searchCondition, params...)
	if err != nil {
		logErrorf(c.Logger(), "searchEstates DB execution error : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func getLowPricedEstate(c *echo.Context) error {
	...
	if err != nil {
		...
		logErrorf(c.Logger(), "getLowPricedEstate DB execution error : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func searchRecommendedEstateWithChair(c *echo.Context) error {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		logInfof(c.Logger(), "Invalid format searchRecommendedEstateWithChair id : %v", err)
		return c.NoContent(http.StatusBadRequest)
	}
	...
	if err != nil {
		if err == sql.ErrNoRows {
			logInfof(c.Logger(), "Requested chair id \"%v\" not found", id)
			return c.NoContent(http.StatusBadRequest)
		}
		logErrorf(c.Logger(), "Database execution error : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func searchEstateNazotte(c *echo.Context) error {
	...
	if err != nil {
		logInfof(c.Echo().Logger, "post search estate nazotte failed : %v", err)
		return c.NoContent(http.StatusBadRequest)
	}
	...
	if err == sql.ErrNoRows {
		logInfof(c.Echo().Logger, "select * from estate where latitude ... : %v", err)
		return c.JSON(http.StatusOK, EstateSearchResponse{Count: 0, Estates: []Estate{}})
	} else if err != nil {
		logErrorf(c.Echo().Logger, "database execution error : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	...
	for _, estate := range estatesInBoundingBox {
		...
		if err != nil {
			if err == sql.ErrNoRows {
				...
				logErrorf(c.Echo().Logger, "db access is failed on executing validate if estate is in polygon : %v", err)
				return c.NoContent(http.StatusInternalServerError)
			}
		} else {
...
func postEstateRequestDocument(c *echo.Context) error {
	m := map[string]any{}
	if err := c.Bind(&m); err != nil {
		logInfof(c.Echo().Logger, "post request document failed : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
	...
	if err != nil {
		...
		logErrorf(c.Logger(), "postEstateRequestDocument DB execution error : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
