...
- `initial_data/*.db` (per-tenant SQLite seed data) ships only via a GitHub Release asset (`bench/Makefile`'s `gh release download`), never as a Git blob in the official repository. It is fetched at AMI-build time. **The exact Release tag/asset name/SHA-256 were not resolved during this implement pass** (network access is out of scope for the `implement` mode of `onboard-kakomon-ami-recipe`); `kakomon12-qualify/scripts/artifact-inputs.env` carries an explicit placeholder that fails closed until a human resolves it during `verify`.
