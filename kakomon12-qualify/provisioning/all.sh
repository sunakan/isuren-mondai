...
# Ubuntu mounts /tmp as a RAM-backed tmpfs. Keep mktemp/unzip/package staging
# on the root-backed volume instead.
RECIPE_TMPDIR=/var/tmp/isuren-mondai-kakomon12-qualify
rm -rf "${RECIPE_TMPDIR}"
...
run_step 71-blackauth-go.sh
run_step 72-bench-build.sh
...
rm -rf "${RECIPE_TMPDIR}"
touch /var/lib/cloud/kakomon12-qualify-provisioned
log "all.sh: done"
